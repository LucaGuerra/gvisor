...
func (e *endpoint) newPassiveHandshake(isn, irs seqnum.Value, opts header.TCPSynOptions, deferAccept time.Duration) *handshake {
	h := e.newHandshake()
	// e.mu is held.
	h.resetToSynRcvd(isn, irs, opts, deferAccept) // +checklocksforce
	return h
}
...
func (e *endpoint) keepaliveTimerExpired() tcpip.Error {
	...
	e.keepalive.unacked++
	e.keepalive.Unlock()
	// Forcing checklocks because e.mu == s.ep.mu.
	e.snd.sendSegmentFromView(buffer.VectorisedView{}, header.TCPFlagAck, e.snd.SndNxt-1) // +checklocksforce
